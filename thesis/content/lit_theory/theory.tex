...
This goal can be more clearly defined through the notion of `non-interference'. A program is said to be non-interfering if, given any two program executions which are identical in terms of their low confidentiality input, the results and behaviour of the executions are indistinguishable \cite{sabelfeld2003if}. Or, equivalently, a program is non-interfering if an external, low-privilege observer cannot infer any high confidentiality information from the program's execution.
