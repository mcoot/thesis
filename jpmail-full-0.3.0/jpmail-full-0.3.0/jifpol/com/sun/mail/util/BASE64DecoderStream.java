/*
 * @(#)BASE64DecoderStream.java	1.10 04/04/02
 *
...
public class BASE64DecoderStream extends FilterInputStream {
     ...
     * controls whether errors in the encoedd data cause an exception
	...
	while (got < 4) {
	    int i = in.read();
	    if (i == -1) {
		if (got == 0 || ignoreErrors)
		    return;	// EOF before any data is ok
		String errstr = "Error in encoded stream: " +
		    "needed 4 valid base64 characters but only got " + got +
		    " before EOF, the valid characters were: \"";
		for (int k = 0; k < got; k++)
		    errstr += (char)decode_buffer[k];
		errstr += "\"";
